...
// 无需图片
export function generateDocxFile(template, fileData) {
  return new Promise((resolve, reject) => {
    getFileBinaryString(template)
      .then((templateData) => {
        const zip = new PizZip(templateData)
        const doc = new DocxTemplater().loadZip(zip).render(fileData)
...
const imageOpts = {
  ...
  getSize: function (img, tagValue, tagName) {
    // img是图片Buffer，tagValue是图片地址，tagName是图片在模板中定义的标签名
    return [150, 150] // [宽, 高]
  },
}

// 有图片
export function generateDocxFileWithImg(template, fileData) {
  return new Promise((resolve, reject) => {
    getFileBinaryString(template)
      .then((templateData) => {
        const zip = new PizZip(templateData)
